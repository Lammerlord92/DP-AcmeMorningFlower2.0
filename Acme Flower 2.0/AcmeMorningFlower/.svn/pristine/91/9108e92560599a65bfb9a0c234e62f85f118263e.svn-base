...
public class FlowerServiceTest extends AbstractTest {
	...
	// The ratio of flowers that are not available.
	@Test
	public void testRatioNotAvaliableFlowers() {
		authenticate("admin1");
		Double ratio = flowerService.notAvaliableFlowers();
	...
	// Trying as unregistered
	@Test(expected = IllegalArgumentException.class)
	public void testRatioNotAvaliableFlowersNegative() {
		Double ratio = flowerService.notAvaliableFlowers();
		Assert.isTrue(ratio > 0);
	}
