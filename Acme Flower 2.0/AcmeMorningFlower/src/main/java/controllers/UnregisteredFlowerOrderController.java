		...
		if(binding.hasErrors()){
			...
			try{
				...
				if(oops.getMessage().equals("expired")){
					result=createEditModelAndView(flowerOrderForm,"EUR","creditCard.expired.error");
				}else if(oops.getMessage().equals("18sender")){
					result=createEditModelAndView(flowerOrderForm,"EUR","sender.18.error");
				}else if(oops.getMessage().equals("18recipient")){
					result=createEditModelAndView(flowerOrderForm,"EUR","recipient.18.error");
				}else
					result=createEditModelAndView(flowerOrderForm,"EUR","flower.commit.error");
			}
		}
		return result;
	}
